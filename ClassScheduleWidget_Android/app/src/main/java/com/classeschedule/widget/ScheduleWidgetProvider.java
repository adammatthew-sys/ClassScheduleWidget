...
public class ScheduleWidgetProvider extends AppWidgetProvider {
    ...
    private static JSONObject next(Context c) {
        try {
            JSONArray a = new JSONArray(c.getSharedPreferences("schedule", 0)
                    .getString("classes", "[]"));
    ...
    static void update(Context c, AppWidgetManager m, int id) {
        ...
        PendingIntent p = PendingIntent.getActivity(
                c, 0, x, PendingIntent.FLAG_IMMUTABLE | PendingIntent.FLAG_UPDATE_CURRENT);

        // widget_root exists in V3 layout; use it as the click target.
